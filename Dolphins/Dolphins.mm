...
OffsetValues offsets[] = {
    { PubgOffset::GWorldFunction, PubgOffset::GWorldData, PubgOffset::GNameFunction, PubgOffset::GNameData },  // Senin 4 offset
...
void *readStaticData(void *) {
    while (true) {
        ...
        if(moduleControl.systemStatus != TransmissionNormal){
            ...
            //角色控制器
            staticData.playerController = memoryTools.readPtr(memoryTools.readPtr(memoryTools.readPtr(staticData.gwlordAddr + PubgOffset::PersistentLevel) + PubgOffset::kNetDriver) + PubgOffset::kServerConnection) + PubgOffset::kPlayerController;
            //掩体判断
            LineOfSightTo = (bool (*)(void *, void *, ImVec3, bool)) (memoryTools.readPtr(memoryTools.readPtr(staticData.playerController + 0x0) + PubgOffset::kLineOfSightTo));
            //自己指针
            staticData.selfAddr = memoryTools.readPtr(staticData.playerController + PubgOffset::kPawn);
            //自瞄函数
            uintptr_t selfFunction = memoryTools.readPtr(staticData.selfAddr + 0);
            AddControllerYawInput = (void (*)(void *, float)) (memoryTools.readPtr(selfFunction + PubgOffset::kYaw));
            AddControllerRollInput = (void (*)(void *, float)) (memoryTools.readPtr(selfFunction + PubgOffset::kRoll));
            AddControllerPitchInput = (void (*)(void *, float)) (memoryTools.readPtr(selfFunction + PubgOffset::kPitch));
            //相机管理器
            staticData.cameraManager = memoryTools.readPtr(staticData.playerController + PubgOffset::kPlayerCameraManager);
            ...
            //遍历地址
            uintptr_t uLevel = memoryTools.readPtr(staticData.gwlordAddr + PubgOffset::kPersistentLevel);
            //数组
            uintptr_t obectArray = memoryTools.readPtr(uLevel + PubgOffset::kActorList);
            //成员数量
            int objectCount = memoryTools.readInt(uLevel + PubgOffset::kActorList + 0x8);
            //开始寻找
            for (int index = 0; index < objectCount; ++index) {
                ...
                //对象坐标指针
                uintptr_t coordAddr = memoryTools.readPtr(objectAddr + PubgOffset::kRootComponent) + PubgOffset::kRelativeLocation;
                ...
                if (strstr(className.c_str(), "PlayerPawn") || strstr(className.c_str(), "PlayerCharacter") || strstr(className.c_str(), "PlayerControllertSl") || strstr(className.c_str(), "_PlayerPawn_TPlanAI_C") || strstr(className.c_str(), "CharacterModelTaget") || strstr(className.c_str(), "FakePlayer_AIPawn") != 0) {
                    //队伍ID
                    int team = memoryTools.readInt(objectAddr + PubgOffset::kMyTeam);
                    int TeamID = memoryTools.readInt(staticData.selfAddr + PubgOffset::kMyTeam);
                    if (team == TeamID) continue;
                    StaticPlayerData tmpPlayerData;
                    //对象指针地址

                    bool bDead = memoryTools.readByte(objectAddr + PubgOffset::kbDead);

                    float hp = memoryTools.readFloat(objectAddr + PubgOffset::kHealth);
                    if (bDead || hp <= 0) continue;
                    uintptr_t statusAddr = memoryTools.readPtr(objectAddr + PubgOffset::kCurrentStates);

                    tmpPlayerData.addr = objectAddr;
                    tmpPlayerData.coordAddr = coordAddr;
                    tmpPlayerData.team = team;
                    tmpPlayerData.name = getPlayerName(memoryTools.readPtr(objectAddr + PubgOffset::kPlayerName));
                    // Bot-gerçek ayrımı (düzeltildi)
                    bool isBot = memoryTools.readByte(objectAddr + PubgOffset::kbIsAI) || memoryTools.readByte(objectAddr + PubgOffset::kbIsMLAI);
                    tmpPlayerData.robot = isBot ? 1 : 0;
                    
                    tmpPlayerData.status = memoryTools.readInt(objectAddr + PubgOffset::kCurrentStates);
...
void readFrameData(ImVec2 screenSize,vector<PlayerData> &playerDataList, vector<MaterialData> &materialDataList) {
    ...
    if (moduleControl.systemStatus == TransmissionNormal) {
        ...
        //取Pov
        MinimalViewInfo pov;
        memoryTools.readMemory(staticData.cameraManager + PubgOffset::kCameraCache, sizeof(pov), &pov);
        //自身坐标
        ImVec3 selfCoord = pov.location;
        //读视角角度
        float lateralAngleView = memoryTools.readFloat(staticData.playerController + PubgOffset::kControlRotation + 0x4) - 90;
        //读取矩阵
        if (moduleControl.mainSwitch.playerStatus) {
            for (auto staticPlayerData: staticData.playerDataList) {
                ...
                //坐标
                ImVec3 objectCoord;
                memoryTools.readMemory(staticPlayerData.coordAddr + PubgOffset::kRelativeLocation, sizeof(ImVec3), &objectCoord);
                //计算自己到对象的距离
                ...
                //获取对象高度
                float objectHeight = memoryTools.readFloat(staticPlayerData.coordAddr + PubgOffset::kHeight);
                if (objectHeight < 20) {
                    continue;
                }
                ...
                //人机 (bot ayrımı burada kullanılıyor)
                ...
                //血量
                playerData.hp = memoryTools.readFloat(staticPlayerData.addr + PubgOffset::kHealth);
                if (playerData.hp > 100) playerData.hp = 100;
                
                //取对手手持武器
                uintptr_t weaponAddr = memoryTools.readPtr(staticPlayerData.addr + PubgOffset::kCurrentWeaponReplicated);
                if (weaponAddr == 0) {
                ...
                uintptr_t meshAddr = memoryTools.readPtr(staticPlayerData.addr + PubgOffset::kMesh);
                uintptr_t boneAddr = meshAddr + 0x30;  // Bone base (senin mesh'ten sonra +0x30 standart UE4)
                BonesData bonesData;
        ...
        if (moduleControl.mainSwitch.materialStatus) {
            for (auto staticMaterialData: staticData.materialDataList) {
                ...
                ImVec3 objectCoord;
                memoryTools.readMemory(staticMaterialData.coordAddr + PubgOffset::kRelativeLocation, sizeof(ImVec3), &objectCoord);
                float objectDistance = get3dDistance(objectCoord, selfCoord, 100);
                ...
                if (staticMaterialData.type == Airdrop) {
                    ...
                    if (get2dDistance(screenSize, goodsListScreen) < 150) {
                        ...
                        uintptr_t goodsListArray = memoryTools.readPtr(staticMaterialData.addr + PubgOffset::kPickUpDataList);
                        int goodsListCount = memoryTools.readInt(staticMaterialData.addr + PubgOffset::kPickUpDataList + sizeof(uintptr_t));
                        for (int index = 0; index < goodsListCount; index++) {
                            if (index > 100) break;
                            int goodsListId = memoryTools.readInt(goodsListArray + PubgOffset::kGoodsID + index * 0x38);
...
void *silenceAimbot(void *) {
    ...
    while (true) {
        ...
        if (moduleControl.systemStatus == TransmissionNormal && moduleControl.mainSwitch.aimbotStatus) {
            // Senin aimbot kodu buraya
        }
    }
    return nullptr;
}

// Diğer fonksiyonlar aynı kaldı (isCoordVisibility, isOnSmoke, getPlayerName, getClassName, getBone, getBone2d)
...
bool isOnSmoke(ImVec3 coord) {
    for (StaticMaterialData smoke: staticData.smokeList) {
        ImVec3 smokeCoord;
        memoryTools.readMemory(smoke.coordAddr + PubgOffset::kRelativeLocation, 30, &smokeCoord);
...
char *getClassName(int classId) {
    ...
    if (classId > 0 && classId < 2000000) {
        ...
        uintptr_t nameAddr = memoryTools.readPtr(pageAddr + index * sizeof(uintptr_t)) + 0x10; // Standart +0x10
