...
// Geliştirilmiş bot kontrolü – hem bIsAI (0xA40) hem de kbIsMLAI (0xA41) kontrol ediliyor
    ...
    if (moduleControl.systemStatus == TransmissionNormal) {
        ...
        if (moduleControl.mainSwitch.playerStatus) {
            for (auto staticPlayerData: staticData.playerDataList) {
                // Çift güvenlik – kendini tekrar atla (zaten listede olmamalı)
                ...
                // ... (statusName ayarları aynen kalır) ...
                // Kısalık için burada kesiyorum, aynı kod devam ediyor.
                // Aşağıdaki kısım değişmemiştir.
                
                // ... (weapon, name, screen, size, skeleton aynen) ...
            }
        }
        // ... (material status aynen) ...
    }
}

// Diğer fonksiyonlar (isCoordVisibility, isOnSmoke, getPlayerName, getClassName, getBone, getBone2d) aynen kalır.
// Aşağıda kısaltılmış olarak verilmiştir. Projenizdeki mevcut kodlarınızı kullanın.